...
Structural node output is atomic. A statement, condition group, or condition must be emitted as
one indivisible bundle containing:

- the node declaration
- the matching structural `categories[]` cache value
- the matching `GRAPH_ROLE` edge

The validation prompt must repair a missing role edge by restoring it. It must not repair the
mismatch by dropping the structural node, clearing its category, or removing the surrounding
conditional subgraph. Deterministic Java completion restores a missing structural role edge as a
last-resort guard; ordinary taxonomy edges are never invented by this guard.
...
- Emit each structural node, its role category, and its `GRAPH_ROLE` edge as one bundle.
...
- Audit every structural bundle in both directions: category to role edge, and role edge to source
  node declaration.
- Restore missing `GRAPH_ROLE` edges instead of deleting valid conditional structure.
