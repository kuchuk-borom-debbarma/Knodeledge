...
Each structural node is an indivisible bundle:

1. The structural node declaration exists.
2. Its `categories` cache contains exactly its role taxonomy ID.
3. A matching `GRAPH_ROLE` edge connects the structural node to that taxonomy node.

For example, a node with `categories: ["statement"]` must have
`statement_node --GRAPH_ROLE--> statement`. The graph patch constructor and auditor are instructed
to preserve all three parts together.

`GraphPatchProcessor.completeReferences` is the deterministic final guard. If an LLM-corrected
patch retains a structural node and its role category but omits the matching `GRAPH_ROLE` edge,
the processor restores that edge before final graph validation. Missing non-structural taxonomy
edges remain errors and are not synthesized.
...
- Structural role metadata cannot silently diverge between node cache and graph edges.
